...
void stestBinaryTreeString(lasd::BinaryTree<string> &bt, uint &testnum, uint &testerr) {
  ...
  try {
    ...
    FoldPreOrder(loctestnum, loctesterr, bt, true, &FoldStringConcatenate, string("?"),
                 string("?ABDC"));
    FoldPostOrder(loctestnum, loctesterr, bt, true, &FoldStringConcatenate, string("?"),
                  string("?DBCA"));
    FoldInOrder(loctestnum, loctesterr, bt, true, &FoldStringConcatenate, string("?"),
                string("?DBAC"));
    FoldBreadth(loctestnum, loctesterr, bt, true, &FoldStringConcatenate, string("?"),
                string("?ABCD"));
  } catch (...) {
    loctestnum++;
    loctesterr++;
...
void testSimpleExercise2A(uint &testnum, uint &testerr) {
  ...
  cout << endl
       << "Exercise 2A (Simple Test) (Errors/Tests: " << testerr << "/" << testnum << ")"
       << endl;
}
