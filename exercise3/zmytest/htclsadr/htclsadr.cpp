...
void my_htclsadr_int(unsigned int &testnum, unsigned int &testerr) {
  unsigned int loctestnum{0};
  unsigned int loctesterr{0};
  cout << endl << "Begin of My HashTable Closed Addressing<int> Test:" << endl;
  try {
  ...
  cout << "End of My HashTable Closed Addressing<int> Test! (Errors/Tests: " << loctesterr
       << "/" << loctestnum << ")" << endl;
  testnum += loctestnum;
  testerr += loctesterr;
}
...
void my_htclsadr_test(unsigned int &testnum, unsigned int &testerr) {
  my_htclsadr_int(testnum, testerr);
}
