...
void my_ht_open_addressing(unsigned int &testnum, unsigned int &testerr) {
  ...
  cout << "End of My Hash Table Open Addressing Test! (Errors/Tests: " << loctesterr
       << "/" << loctestnum << ")" << endl;
  testnum += loctestnum;
  testerr += loctesterr;
}
...
void my_ht_open_addressing_test(unsigned int &testnum, unsigned int &testerr) {
  my_ht_open_addressing(testnum, testerr);
}
